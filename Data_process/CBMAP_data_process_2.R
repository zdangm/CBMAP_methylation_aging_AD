...
setwd('/methylation/data/CBMAP/DNAm_processed/zk_DNAm_processed')
## Step 1: read idat files
indir <- "/methylation/data/CBMAP/DNAm_processed/mirror_raw_data_DNAm/zk/"
gfile <- iadd2('/methylation/data/CBMAP/DNAm_processed/mirror_raw_data_DNAm/zk', gds = 'zk_DNAm.gds', chunksize = 100)
write.table(paste("dim_rawData:", paste(dim(betas(gfile)[,]), collapse="*")), file="zk_log.DNAm_QC", row.names=F, ,quote=F, col.names=F, append=F)
...
write.table(paste("dim_rmDuplicatedProbe:", paste(dim(betas(gfile)[,]), collapse="*")), file="zk_log.DNAm_QC", row.names=F, ,quote=F, col.names=F, append=T)
...
write.table(paste("dim_rmLowQual:", paste(dim(betas(gfile)[,]), collapse="*")), file="zk_log.DNAm_QC", row.names=F, quote=F, col.names=F, append=T)
...
write.table(paste("nCpG_rmSNPprobe:", nCpG_rmSNPprobe), file="zk_log.DNAm_QC", row.names=F, quote=F, col.names=F, append=T)
write.table(paste("nCpG_rmxreact:", nCpG_rmxreact), file="zk_log.DNAm_QC", row.names=F, quote=F, col.names=F, append=T)
write.table(paste("nCpG_rm5bpSNP:", nCpG_rm5bpSNP), file="zk_log.DNAm_QC", row.names=F, quote=F, col.names=F, append=T)
write.table(paste("dim_rmSNP_xreact:", paste(dim(betas(gfile)[,]), collapse="*")), file="zk_log.DNAm_QC", row.names=F, quote=F, col.names=F, append=T)
...
write.table(paste("dim_rmFinalmissing:", paste(dim(betas(gfile)[,]), collapse="*")), file="zk_log.DNAm_QC", row.names=F, quote=F, col.names=F, append=T)


## Step 4: predict sex
idatlist <- sub("_Red.idat", "", paste0('/methylation/data/CBMAP/DNAm_processed/mirror_raw_data_DNAm/zk/', list.files(path='/data/projects/China_Brain_MultiOmics/methylation/data/CBMAP/DNAm_processed/mirror_raw_data_DNAm/zk', pattern="*_Red.idat", recursive=T)))
XY <- t(sapply(
  idatlist,
  function(infile){
...
write.table(paste("dim_rmSexMismatch:", paste(dim(betas(gfile)[,]), collapse="*")), file="zk_log.DNAm_QC", row.names=F, quote=F, col.names=F, append=T)
