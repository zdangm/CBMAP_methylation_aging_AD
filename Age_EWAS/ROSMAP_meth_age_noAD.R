...
load('/methylation/results/disease_meth_age/AD_meth/ROSMAP_pheno.RData')
...
rin_info = read.csv('/ROSMAP/Processed/metadata/ROSMAP_assay_rnaSeq_metadata.csv',header=T)
...
cell.pro = read.table('/methylation/results/ctp_analysis/ROSMAP_all_sample_methylation_ctp_with_clr_deconvolution.txt',header=T)
...
pheno_df <- pheno_df %>% select(age_death, msex, Study, pmi, rin, Exc, Inh, NonN_Astro_FGF3R, NonN_Endo, NonN_Micro, NonN_Oligo_MBP, NonN_OPC, Sample_Plate, batch)
apply(pheno_df, 2, function(x){sum(is.na(x))})
# age_death             msex            Study              pmi              rin 
#         0                0                0                2              101 
# Exc              Inh NonN_Astro_FGF3R        NonN_Endo       NonN_Micro 
#   8                8                8                8                8 
# NonN_Oligo_MBP         NonN_OPC     Sample_Plate  
#              8                8                0  
...
run_linear_regression <- function(methylation_data, covariates) {
  ...
  model <- lm(methylation_level ~ age_death + msex + Study + pmi + rin + Exc + Inh + NonN_Astro_FGF3R + NonN_Endo + NonN_Micro + NonN_Oligo_MBP + Sample_Plate, data = data)
...
analyze_methylation <- function(cg, pheno_df) {
  results <- apply(cg, 2, function(x) {
    methylation_data <- x  
    covariates <- pheno_df %>% select(age_death, msex, Study, pmi, rin, Exc, Inh, NonN_Astro_FGF3R, NonN_Endo, NonN_Micro, NonN_Oligo_MBP, NonN_OPC, Sample_Plate)  
