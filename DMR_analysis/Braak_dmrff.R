...
sample = intersect(rownames(ros_cg), pheno_df$mwas_id) # 718 samples
ros_cg = ros_cg[sample,]
ros_cg = ros_cg[,grep("cg",colnames(ros_cg))]
...
sample = Reduce(intersect,list(rownames(cb_cg),braak_sample,rownames(pheno_df)))
cb_cg <- cb_cg[sample,]
...
save(ros_dmr_pre,cb_dmr_pre,meta_dmr,file = 'braak_dmrff.RData')
dmrs <- meta_dmr$dmrs[which(meta_dmr$dmrs$p.adjust < 0.05 & meta_dmr$dmrs$n >= 3), ]
write.csv(dmrs,'braak_sig_dmr.csv')
