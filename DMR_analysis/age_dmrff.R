...
###dmrff
age_dmr <- dmrff(estimate = result$Estimate,se=result$StdErr,p.value = as.vector(result$p_bacon),methylation = cg,chr=result$CHR,pos=as.numeric(result$MAPINFO))
age_dmr_sig <- age_dmr[age_dmr$p.adjust < 0.05 & age_dmr$n >= 2, ]#15416 regions
