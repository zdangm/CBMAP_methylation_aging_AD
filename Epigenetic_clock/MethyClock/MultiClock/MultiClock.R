MultiClock<-function(betas, ## betas = betas matrix (rownames=cpgs, colnames=IDs)
  ...
  braincoef<-read.csv(paste0(dir,"13059_2013_3156_MOESM3_ESM.csv"),stringsAsFactor=F,header=F)
  braincoef<-braincoef[-c(1,2),]
  colnames(braincoef)<-braincoef[1,]
  braincoef<-braincoef[-1,]
  braincoef<-braincoef[,c('CpGmarker','CoefficientTraining')]
  ...
  brainpred<-braincoef$coef%*%betas+0.695507258
  ...
  compareStats<-function(data){
    colnames(data)<-c("ID","Age","Multi Clock")
  ...
  plotAge<-function(data){
    ...
    ggsave("MultiClockplot.pdf", height= 5, width=6)
  }
  
  plotAge(pheno)
  
  print("Your Multi DNAm ages are saved in the file 'MultiPred.csv'")
  print("Accuracy statistics comparing DNAm age and chronological age are saved in the file 'accuracy_statistics.csv'")
  print("Plot of Multi DNAm age against age is saved in the file 'MultiClockplot.pdf'")
