HannumClock<-function(betas, ## betas = betas matrix (rownames=cpgs, colnames=IDs)
  ...
  braincoef<-read_excel(paste0(dir,"mmc2.xlsx"))[,c('Marker','Coefficient')]
  ...
  brainpred<-braincoef$coef%*%betas
  ...
  write.csv(pheno, "HannumPred.csv")
  ...
  compareStats<-function(data){
    colnames(data)<-c("ID","Age","Hannum Clock")
  ...
  plotAge<-function(data){
    ...
    ggsave("HannumClockplot.pdf", height= 5, width=6)
  }
  
  plotAge(pheno)
  
  print("Your Hannum DNAm ages are saved in the file 'HannumPred.csv'")
  print("Accuracy statistics comparing DNAm age and chronological age are saved in the file 'accuracy_statistics.csv'")
  print("Plot of Hannum DNAm age against age is saved in the file 'HannumClockplot.pdf'")
