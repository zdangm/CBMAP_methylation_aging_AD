iCAS_DNAm<-function(betas, ## betas = betas matrix (rownames=cpgs, colnames=IDs)
  ...
  braincoef<-read_excel(paste0(dir,"pwae011_suppl_supplementary_tables_s7.xlsx"))
  colnames(braincoef)<-braincoef[1,]
  braincoef<-braincoef[-1,]
  braincoef<-braincoef[which(braincoef$'DNAm-based clock type'=='iCAS-DNAmAge'),c('Variable','Coefs')]
  braincoef <- as.data.frame(braincoef)
  colnames(braincoef) <- c('probe','coef')
  braincoef$probe<-as.character(braincoef$probe)
  braincoef$coef<-as.numeric(braincoef$coef)
  ...
  brainpred<-braincoef$coef%*%betas + 2.35192452963228
  ...
  compareStats<-function(data){
    colnames(data)<-c("ID","Age","iCAS-DNA")
  ...
  plotAge<-function(data){
    ...
    ggsave("iCASClockplot.pdf", height= 5, width=6)
  }
  
  plotAge(pheno)
  
  print("Your iCAS DNAm ages are saved in the file 'iCASPred.csv'")
  print("Accuracy statistics comparing DNAm age and chronological age are saved in the file 'accuracy_statistics.csv'")
  print("Plot of iCAS DNAm age against age is saved in the file 'iCASClockplot.pdf'")
