...
pheno_df$braaksc <- as.numeric(pheno_df$braaksc)
...
pheno_df <- pheno_df %>% select(braaksc, age_death, msex, Study, pmi, rin, Exc, Inh, NonN_Astro_FGF3R, NonN_Endo, NonN_Micro, NonN_Oligo_MBP, NonN_OPC, Sample_Plate)
apply(pheno_df, 2, function(x){sum(is.na(x))})
# braaksc        age_death             msex            Study              pmi 
#       0                0                0                0                3 
...
run_linear_regression <- function(methylation_data, covariates) {
  ...
  model <- lm(methylation_level ~ braaksc + msex + age_death + Study + pmi + rin + Exc + Inh + NonN_Astro_FGF3R + NonN_Endo + NonN_Micro + NonN_Oligo_MBP + Sample_Plate, data = data)
...
analyze_methylation <- function(cg, pheno_df) {
  results <- apply(cg, 2, function(x) {
    methylation_data <- x  
    covariates <- pheno_df %>% select(braaksc, age_death, msex, Study, pmi, rin, Exc, Inh, NonN_Astro_FGF3R, NonN_Endo, NonN_Micro, NonN_Oligo_MBP, NonN_OPC, Sample_Plate)  
...
save(results, file='ROSMAP_meth_braak.RData')
