...
res = vector('list',9)
o = 0
for ( GS.type in c("GO","Reactome","KEGG")) {
  for (j in 1:length(cpg_list)) {
    ...
    res1 = methylRRA(cpg.pval = sig.cpg, array.type = "450K", method = "GSEA", minsize = 5, maxsize = 500, GS.type = GS.type)
    res1.sig<-res1[res1$padj<0.05,]
    res[[o]] = res1.sig
    names(res)[o] = paste0(group,'_',GS.type)
  }
}
names(res)[is.na(names(res))] <- paste0("unknown_", seq_len(sum(is.na(names(res)))))
write.xlsx(res,'methylGSA_methylRRA_GSEA.sig.xlsx')
