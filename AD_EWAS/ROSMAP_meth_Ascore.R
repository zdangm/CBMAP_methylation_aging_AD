...
pheno_df$Ascore <- as.numeric(unlist(hml_info[match(pheno_df$projid, hml_info$projid), "a_score_st4"]))
...
pheno_df <- pheno_df[!is.na(pheno_df$Ascore),]
...
# methylation matrix
cg <- h5read("/data/ROSMAP_methylaition/final_DNAm_invMdat.h5",'Mdat')
colnames(cg) = h5read("/data/ROSMAP_methylaition/final_DNAm_invMdat.h5", 'colnames')
rownames(cg) = h5read("/data/ROSMAP_methylaition/final_DNAm_invMdat.h5", 'rownames')
sample = intersect(rownames(cg), pheno_df$mwas_id) 
cg = cg[sample,]
cg = cg[,grep("cg",colnames(cg))] 
pheno_df = pheno_df[sample,]
pheno_df <- pheno_df %>% select(Ascore, age_death, msex, Study, pmi, rin, Exc, Inh, NonN_Astro_FGF3R, NonN_Endo, NonN_Micro, NonN_Oligo_MBP, NonN_OPC, Sample_Plate)
apply(pheno_df, 2, function(x){sum(is.na(x))})
# Ascore        age_death             msex            Study              pmi 
#     0                0                0                0                3 
...
run_linear_regression <- function(methylation_data, covariates) {
  ...
  model <- lm(methylation_level ~ Ascore + msex + age_death + Study + pmi + rin + Exc + Inh + NonN_Astro_FGF3R + NonN_Endo + NonN_Micro + NonN_Oligo_MBP + Sample_Plate, data = data)
...
analyze_methylation <- function(cg, pheno_df) {
  results <- apply(cg, 2, function(x) {
    methylation_data <- x  
    covariates <- pheno_df %>% select(Ascore, age_death, msex, Study, pmi, rin, Exc, Inh, NonN_Astro_FGF3R, NonN_Endo, NonN_Micro, NonN_Oligo_MBP, NonN_OPC, Sample_Plate)  
...
save(results, file='ROSMAP_meth_Ascore.RData')