...
selected_sample <- selected_sample %>% 
  filter(
    diag_ALS == 0 & 
      ...
      diag_others == 0 &
      OTHER == 0) 
braak_sample <- unlist(selected_sample %>% filter(!is.na(Braak.NFT.stage))%>% dplyr::select(id)) 
...
pheno_df$AD <- factor(pheno_df$Braak.NFT.stage,levels = c(0:6),ordered = T) 
...
for (i in 1:ncol(cg)) {
  fit1 = lm(cg[,i] ~ age + sex_male + bank + PMD + RIN + Exc + Inh + NonN_Astro_FGF3R + NonN_Endo + NonN_Micro + NonN_Oligo_MBP + Sample_Plate, data = pheno_df)
  fit1 = summary(fit1)
...
res_M = res_M[-1,]
##### AD ~ age #####
fit3 = polr(AD ~ age + sex_male + bank + PMD + RIN + Exc + Inh + NonN_Astro_FGF3R + NonN_Endo + NonN_Micro + NonN_Oligo_MBP + Sample_Plate, data = pheno_df,
...
for (i in 1:ncol(cg)) {
  fit2 = polr(AD ~ age + cg[,i] + sex_male + bank + PMD + RIN + Exc + Inh + NonN_Astro_FGF3R + NonN_Endo + NonN_Micro + NonN_Oligo_MBP + Sample_Plate, data = pheno_df,
              Hess = TRUE, method = 'logistic')
  fit2 = summary(fit2)
...
sum(res$fdr < 0.05)
write.table(res,file='mediation_braak_res.txt',col.names = T,row.names = F,quote = F,sep='\t')