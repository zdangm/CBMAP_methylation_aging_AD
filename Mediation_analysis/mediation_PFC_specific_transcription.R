...
brain_specific_cpg <- read_xlsx("brain_specific_cpg.xlsx")
brain_specific_cpg <- brain_specific_cpg$cpg

###cpg-gene
cpg_gene_res = as.data.frame(fread("cis_eQTM_all_res_clr.txt",header=T))
cpg_gene_res <- cpg_gene_res[cpg_gene_res$cpg %in% brain_specific_cpg, ]
...
load('/data/projects/China_Brain_MultiOmics/methylation/results/disease_meth_age/AD_meth/CBMAP_pheno.RData')
cell.pro = read.table('/data/projects/China_Brain_MultiOmics/methylation/results/ctp_analysis/CBMAP_all_sample_methylation_ctp_with_clr_deconvolution.txt',header=T)
cell_aligned <- cell.pro[pheno_df$barcode_id, , drop = FALSE]
pheno_df <- cbind(pheno_df, cell_aligned)
...
write.csv(combined_all_filtered,'pfc_speci_association_direction.csv')
...
mediation_res <- data.frame('cpg' = rep(NA, nrow(combined_all_filtered)), 
                            'Mediated.Prop' = rep(NA, nrow(combined_all_filtered)), 
                            'estimate' = rep(NA, nrow(combined_all_filtered)), 
                            'P' = rep(NA, nrow(combined_all_filtered)))
for (i in 1:nrow(combined_all_filtered)) {
  cpg <- as.character(combined_all_filtered[i, "cpg"])
  gene <- as.character(combined_all_filtered[i, "gene_id"])
  ...
  dat = data.frame('methy' = dat[,2], 'age' = dat$age, 'exp' = dat[,3], 'sex' = dat$sex_male, 'rin' = dat$RIN, 'pmd' = dat$PMD,'bank'=dat$bank, 'Exc'=dat$Exc,
                   'Inh'=dat$Inh,  'NonN_Astro_FGF3R'=dat$NonN_Astro_FGF3R,'NonN_Endo'=dat$NonN_Endo,'NonN_Micro'=dat$NonN_Micro, 'NonN_Oligo_MBP'=dat$NonN_Oligo_MBP, 'Sample_Plate'=dat$Sample_Plate)
...
original_data <- read.csv("pfc_speci_association_direction.csv")
...
res_table <- res_table[,-13]
med_sig <- res_table[res_table$med_fdr<0.05,]
write.xlsx(med_sig,'pfc_specific_med_sig.xlsx')
